...
\newcommand{\drawskillbars}[1]{
    \begin{tikzpicture}
        ...
        \foreach \i in {0, 1, 2, 3, 4}{
            \fill[lightgray] (\i * 0.4 + 0.1 * \i, 0) rectangle (0.4 + \i * 0.4 + \i * 0.1, 0.1);
        }
        
        % Draw number of black bars depending on the skill level
        \foreach \i in {#1}{
            \fill[black] (\i * 0.4 + 0.1 * \i, 0) rectangle (0.4 + \i * 0.4 + \i * 0.1, 0.1);
        }
    \end{tikzpicture} \par
}
...
\newcommand{\education}[5]{
    % Name of the studies
    \noindent \large \parbox{.75\linewidth}{\textbf{#1}}
    % Duration in a Box
    \hfill \scriptsize
    \tcbox[enhanced,box align=base,nobeforeafter,colback=title,colframe=title,size=fbox,arc=0mm]{\textbf{#2}} 
    \vspace{0.3em}
    % School Name 
    \large
    \noindent \color{black} \parbox{.7\linewidth}{\textsl{\normalsize{#3}}} \hfill \scriptsize #4 \par
    % Description
...
\newcommand{\work}[4]{
    ...
    \noindent \large \color{black} \parbox{.7\linewidth}{\textsl{\normalsize{#3}}} \par
